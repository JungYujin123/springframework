   	...
   	<section class="content">
   		<div class="card">
   			<div class="card-header with-border">
   		 		<button type="button" class="btn btn-primary" onclick="window.open('insert','width=800,height=900');" >회원등록</button>
    		  ...
    		  <div class="row">
	             <div class="col-sm-12">	
		    		<table class="table table-bordered">
		               ...
		               <!-- 	MemberListAction에서 같은 req 공유하고 있어 가져옴 var="member"은 지금 지정, member.id 용 -->
		            	<c:if test="${not empty memberList }" >
			               	<c:forEach items="${memberList }" var="member" >
			               	  <tr  onclick="detail_go('${member.id}');" style="cursor:pointer;">
			               		<td>${member.id }</td>
			              		<td>${member.pwd }</td>
			              		<td>${member.name }</td>
			              		...
			              		<td>
			              			<%-- <%
			              				Date regDate = ((MemberVO)pageContext.getAttribute("member")).getRegDate();
			              				String dateStr=new SimpleDateFormat("yyyy-MM-dd").format(regDate);
			              				out.print(dateStr);
			              			%> --%>
...
<script>
	function detail_go(member_id){
		//alert(member_id);
		window.open('detail?id='+member_id,'회원상세','width=700,height=800');
	}
	</script>
