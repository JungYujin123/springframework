...
<form role="imageForm" action="upload/picture" method="post" enctype="multipart/form-data">
   <input id="inputFile" name="pictureFile" type="file" class="form-controll" onchange="picture_go();" style="display: none;" />
...
<script>
   ...
   /* processData:false,
      contentType:false, 이 들어가야 default값이 들어가지 않는다! */
   $.ajax({
      url:"picture.do",
      data:formData,
      type:'post',
      ...
      $.ajax({
         ...
         success : function(result){
            if(result.toUpperCase() == "DUPLICATED"){
               ...
               alert("사용가능한 아이디 입니다.");
               checkedID=input_ID.val(input_ID.val().trim());
            }
         },
         error:function(error){
